document.addEventListener('DOMContentLoaded', () => {
    ...
    const REQUIRED_CLICKS = 50; // 30-100 aralığında belirlendi
    ...
    for (let i = 1; i <= TOTAL_TEAMS; i++) {
        initialScores.push({ 
            rank: `#${i < 10 ? '0' + i : i}`, 
            team: `TAKIM ADI ${i}`, 
            color: '#1E90FF', // Lacivert Vurgu Rengi
            total: '0' 
        });
    }
    ...
    function renderScoreboard(isEditable) {
        ...
        for (let i = 0; i < half; i++) {
            ...
            const teamColorL = leftData.color || '#1E90FF';
            ...
            if (rightIndex < TOTAL_TEAMS) {
                ...
                const teamColorR = rightData.color || '#1E90FF';
        ...
        if (isEditable) {
            document.querySelectorAll('.editable').forEach(cell => {
                ...
                if (cell.classList.contains('color-editable')) {
                    ...
                    cell.textContent = scores[index].color || '#1E90FF';
                }
            });
        } else {
